...
type LoggedIn = { state: "LoggedIn", auth: UserAuthorization, activeTimeout: number }
type LoadingRefresh = { state: "LoadingRefresh", auth: UserAuthorization, abortController: AbortController }
...
export function AuthProvider(props: AuthContextProps) {
    ...
    let [internalState, setInternalState] = createSignal<InternalAuthState>({state: "LoggedOut"});
    ...
    let contextValue: AuthContextData = {auth, loading, error, login, logout, setAuth, returnTo, setReturnTo};
